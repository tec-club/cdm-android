package com.ampelement.cdm;

public class Preferences {
	
	public static final String SCHOOL_LOOP_USERNAME = "schoolLoopUsername";
	public static final String SCHOOL_LOOP_PASSWORD = "schoolLoopPassword";
	public static final String SCHOOL_LOOP_SLID = "loginSLID";
	public static final String SCHOOL_LOOP_JSESSIONID = "loginJSESSIONID";
	public static final String SCHOOL_LOOP_TIME = "lastActiveTime";
	
	// Club JSON data
	public static final String CLUB_CACHED_DATA = "cachedClubData";
	public static final String CLUB_CACHED_DATA_VERSION = "cachedClubDataVersion";
	
	// Calendar Event XML data
	public static final String CALENDAR_CACHED_DATA = "cachedCalendarData";
	
	// Settings
	public static final String SETTINGS_ALLOW_PUSH_NOTIFS = "allowPushNotifs";

}
